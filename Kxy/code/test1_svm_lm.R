...
library(Rdonlp2)
library(leaps)
train_filenames <- list.files(path = "E:\\KDD_CUP_2017\\dataSets\\dataSets\\train1_kxy_allsplit",pattern = ".csv",full.names = T)
test_filenames <- list.files(path = "E:\\KDD_CUP_2017\\dataSets\\dataSets\\test1_kxy_allsplit",pattern = ".csv",full.names = T)
test_filenames
train_filenames
...
for(i in 1:6){
  ...
  # dotchart(train$avg_travel_time)#绘制单变量散点图,多兰图  
  # pc=boxplot(train$avg_travel_time,horizontal=T)#绘制水平箱形图 
  q99 <- quantile(train$avg_travel_time,0.99)
  q1 <- quantile(train$avg_travel_time,0.001)
  train <- train[(train$avg_travel_time < q99)&(train$avg_travel_time > q1),]
  #########################################
  dimnames(train)[2]
  
  train <- normalization(train)
  train <- train[,-c((ncol(train)-11):ncol(train))]
  train <- subset.data.frame(train,select = -c(intersection_id,tollgate_id,time_window,holiday,pressure))
  train$avg_travel_time <- log(sqrt(train$avg_travel_time))
  ...
  test <- subset.data.frame(test,select = -c(intersection_id,tollgate_id,time_window,holiday,pressure))

    
  ###############select attribute#####################
  regfit.full <- regsubsets(avg_travel_time~.,train,nvmax = ncol(train))
  ...
  for(j in 1:k){
    best.fit <- regsubsets(avg_travel_time~.,train[folds!=j,],nvmax = ncol(train))
    ...
    for( m in 1:length(reg.summary$rsq)){
      ...
      cv.errors[j,m] <- mean(abs((train$avg_travel_time[folds==j]-pred)/train$avg_travel_time[folds==j]))
    }
  }
  ...
  test.mat <- model.matrix(avg_travel_time~.,data = test)
  lm_predictions <- test.mat[,names(coefi)]%*%coefi
  ##################################################
  # lm_fit <- lm(avg_travel_time~.,data = train[,c("avg_travel_time",names(coefi)[-1])])
  # summary(lm_fit)
  # lm_fit <- lm(avg_travel_time~.,data = train)
  # lm_predictions<-predict(lm_fit,newdata=test)
  
  svm_fit <- svm(avg_travel_time~.,data = train)
  svm_predictions<-predict(svm_fit,newdata=test)
  
  rf_fit<-randomForest(avg_travel_time~.,data=train,mtry = floor((ncol(train))/3),importance = T)
  ...
  #     lm_fit<-lm(avg_travel_time~.,data=training)
  #     svm_fit <- svm(avg_travel_time~.,data = training)
  # 
  ...
  #     error<-mean(abs(testing$avg_travel_time-predictions3)/testing$avg_travel_time)
  #     return(error)
  #   }
...
dim(final_result_rf)
sum(final_result$intersection_id=="C")

write.csv(final_result_lm,file = "E:\\KDD_CUP_2017\\dataSets\\dataSets\\test1_alldata_nosplit_lm_noabnormal_cvselect_result.csv",row.names = F)
write.csv(final_result_svm,file = "E:\\KDD_CUP_2017\\dataSets\\dataSets\\test1_alldata_nosplit_svm_noabnormal_cvselect_result.csv",row.names = F)
write.csv(final_result_rf,file = "E:\\KDD_CUP_2017\\dataSets\\dataSets\\test1_alldata_nosplit_rf_noabnormal_cvselect_result.csv",row.names = F)
...
write.csv(final_result_lmsvm,file = "E:\\KDD_CUP_2017\\dataSets\\dataSets\\test1_alldata_nosplit_lmsvm_noabnormal_cvselect_result.csv",row.names = F)
write.csv(final_result_svmrf,file = "E:\\KDD_CUP_2017\\dataSets\\dataSets\\test1_alldata_nosplit_svmrf_noabnormal_cvselect_result.csv",row.names = F)
write.csv(final_result_lmrf,file = "E:\\KDD_CUP_2017\\dataSets\\dataSets\\test1_alldata_nosplit_lmrf_noabnormal_cvselect_result.csv",row.names = F)
write.csv(final_result_lmsvmrf,file = "E:\\KDD_CUP_2017\\dataSets\\dataSets\\test1_alldata_nosplit_lmsvmrf_noabnormal_cvselect_result.csv",row.names = F)
